...
public:
    ...
    double sigm_pro(double x) { //Производная от сигмойды
            ...
            if (i < n - 1) {
                ...
                for (int j = 0; j < p[i]; j++) {
                    ...
                    for (int k = 0; k < p[i + 1]; k++) {
                        fin >> weights[i][j][k];    //загружаем веса из файла.
                        summ += weights[i][j][k];
                    }
                }
            }
        ...
        for (int i = 0; i < n; i++) {
            ...
            if (i < n - 1) {
                ...
                for (int j = 0; j < p[i]; j++) {
                    weights[i][j] = new double[p[i + 1]];
                    for (int k = 0; k < p[i + 1]; k++) {
                        weights[i][j][k] = ((rand() % 100)) * 0.01 / size[i];   //Создаем рандомные веса
                    }
                }
            }
        }
    ...
    bool SaveWeights() {
        ...
        for (int i = 0; i < layers; i++) {
            if (i < layers - 1) {
                for (int j = 0; j < size[i]; j++) {
                    for (int k = 0; k < size[i + 1]; k++) {
                        fout << weights[i][j][k] << " ";    //сохраняем веса
                    }
                }
            }
        }
...
int main() {
    ...
    ofstream fout;
    fout.open("log.txt");
    const int l = 4;    //кол-во внутренних слоев
    ...
    int maxra = 0;
    int maxraepoch = 0;
    const int n = 83;   // Кол-во данных для обучения
    bool to_study = 0;
    std::cout << "Getting study?";
    std::cin >> to_study;
    double time = 0;
    ...
    if (to_study) {
        ...
        for (int i = 0; i < n; i++){
            for (int j = 0; j < input_l; j++) {
                fin >> data[i].info[j]; //считываем базу для обучения
            }
            fin >> data[i].rresult;
            cout << data[i].rresult;
            data[i].rresult -= 65;
    ...
    while(true)
    {
        ...
        if (to_start_test) {
            ...
            cout << "I think this is " << char(result + 65) << "\n\n";
            cout << "What is this...";
            cin >> right_res;
            if (right_res != result) {
                cout << "Fix problem";
